...
def write_table_v(paired, tost, path):
    ...
    lines = [
        ...
        r"\toprule",
        r"Comparison & Median $\Delta$ & 95\% BCa CI & Holm $p$ & $r_{rb}$ & 90\% CI (TOST) & Equiv.\ ($\delta{=}0.02$) \\",
        r"\midrule",
    ]
    for r in paired:
        ...
        lines.append(
            ...
            f"{r['p_holm']:.3f} & {r['rank_biserial']:+.2f} & "
            f"[{t['ci90_bca_low']:+.3f}, {t['ci90_bca_high']:+.3f}] & {eq} \\\\"
        )
...
def main() -> int:
    ...
    write_table_v(paired_rows, tost_rows, ROOT / "manuscript" / "tables" / "table_V_paired_comparisons.tex")
    try:
