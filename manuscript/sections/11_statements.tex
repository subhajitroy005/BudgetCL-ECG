...
\vspace{0.3em}
\noindent
The release tag is the primary identifier: a manuscript cannot contain its own
Git commit hash, because inserting the hash changes the commit. The exact commit
for this release is recorded in \texttt{releases/release\_manifest.json} inside
the tagged tree, so \texttt{v1.0.0-arxiv} resolves to it unambiguously.
