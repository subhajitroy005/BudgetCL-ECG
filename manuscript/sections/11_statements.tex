...
to reproduce every \emph{reported} number in this paper \emph{from the
released run artifacts}. Every published value is regenerated from those
artifacts by a single driver (\texttt{scripts/reproduce\_paper.py}); no result
is transcribed by hand. \textbf{This is reproduction of the reported analysis,
not re-execution of the experiments}: the driver starts from the released
per-patient and per-seed result files and does not retrain the models, which
would require the PhysioNet recordings and several GPU-hours.
