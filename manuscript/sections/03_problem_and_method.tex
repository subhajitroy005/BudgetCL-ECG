...
\paragraph{Persistent versus transient state, argued.} Excluding the
forward/backward working set is a modelling choice, and a reviewer is right to
ask whether it was chosen to favour a conclusion. We therefore argue the split
rather than assume it, using a byte-exact analytical liveness account of the
adaptation graph (no hardware quantity is measured). The two axes carry
\emph{separate shadow prices}. Persistent state is a statically reserved region
whose ceiling is the 16\,KiB budget. Peak \emph{transient} state is the maximum
simultaneously-live activation and gradient working set during one
forward$+$backward step; it can be aliased onto the same arena the frozen model
already provisions for inference, so it is charged differently. Crucially, the
two axes are \emph{decoupled by the replay-point cliff}: post-pool replay stores
the pooled vector, so head and post-pool-adapter adaptation (A1--A3) never run
the encoder during a step and their peak transient is a fraction of a KiB
(under $0.3\bytes\mathrm{K}$; $0.1\kib$ in a batch-1 FP16 regime). Raw replay
(A4--A5) must run the full encoder forward and backward, so its transient is
dominated by the $66\times128 = 8{,}448$-element feed-forward tensor and its
gradient: $154.6\kib$ naive FP32, $77.3\kib$ at batch-1 FP16, and $41.3\kib$
under batch-1 FP16 with block rematerialisation (compute multiplier
$1.99\times$ forward FLOPs). That is a $\sim$300$\times$ asymmetry on the
excluded axis. It does not vanish under rematerialisation, but it is partly
\emph{aliasable} onto the inference arena, and how much depends on the precision
one compares against. We state both ratios rather than only the flattering one.
\emph{Element-for-element (topology).} The rematerialised adaptation working set
is $21{,}156$ live elements against the frozen forward pass's $16{,}896$, so
adaptation needs only $\approx\!1.25\times$ as many live elements as inference
already touches; at matched FP16 precision the inference arena is $33.0\kib$ and
the adaptation transient $41.3\kib$, the same $1.25\times$. \emph{Deployed
reality.} A device that ships this classifier runs \emph{inference} at INT8---the
replay buffer is stored INT8 for exactly this reason
(\texttt{replay\_precision=int8})---so the INT8 arena it actually provisions is
$16{,}896\bytes$ ($16.5\kib$), whereas an adaptation step runs at FP16 and peaks
at $42{,}312\bytes$ ($41.3\kib$): a firmware architect pays
$\mathbf{2.50\times}$ the deployed inference arena, not $1.25\times$. The
residual charge beyond the INT8 arena is the FP16 feed-forward-hidden gradient.
Claiming only the $1.25\times$ figure would understate what the two-axis analysis
exists to expose. We report all three regimes rather than the single most
favourable one; the full per-arm, per-regime table is in
Appendix~\ref{sec:embedded} and the source is
\texttt{results/e6\_two\_axis\_accounting.csv}.
