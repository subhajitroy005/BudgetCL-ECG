...
excluded axis. It does not vanish under rematerialisation, but it is largely
\emph{aliasable}: the frozen-model inference arena is itself $33.0\kib$ at FP16
(it holds the same feed-forward tensor), so the encoder arms' adaptation
transient is only $\approx1.25\times$ the arena the device already reserves.
The residual charge beyond inference is the feed-forward-hidden gradient. We
report all three regimes rather than the single most favourable one; the full
per-arm, per-regime table is in Appendix~\ref{sec:embedded} and the source is
